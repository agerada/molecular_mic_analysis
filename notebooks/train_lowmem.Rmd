...
if (!interactive) {
  option_list <- list(
    ...
    make_option(c("-f", "--folds"), type = "double", default = 5,
                help = "Number of folds for cross-validation [default = 5],
                or fraction for train-test split"),
...
# convert folds to integer
# but if < 1, will be treated as a fraction for train-test split
if (opt$folds > 1) {
...
for (i in seq_along(all_abx)) {
  ...
  model_log_kmer[[abx]] <- xgb.cv.lowmem(
    data = dtrain,
...
for (i in seq_along(all_abx)) {
  ...
  model_log_annots[[abx]] <- xgb.cv.lowmem(
    data = dtrain,
