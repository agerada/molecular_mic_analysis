...
with_progress({
    ...
    target_dir = file.path(output_dir, paste0("kmers_", opt$kmer)),
    k = opt$kmer,
    canonical = TRUE,
    squeeze = opt$squeeze)},
  enable = TRUE
      ...
      with_progress({
        MIC::split_and_combine_files(
          ...
          train_target_path = file.path(
            output_dir, paste0("kmers_", opt$kmer), abx, "train.txt"),
          test_target_path = file.path(
            output_dir, paste0("kmers_", opt$kmer), abx, "test.txt"),
          names_backup = file.path(
            output_dir, paste0("kmers_", opt$kmer), abx, "names.csv"),
          overwrite = FALSE)
      }, enable = TRUE)
