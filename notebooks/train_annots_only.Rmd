...
if (!is.null(opt$pre_fold_info)) {
  ...
  if (!all(all_abx %in% names(folds_train_genome_names))) {
    stop("Some antibiotics are missing in the pre-fold information")
  }
  folds_train <- list()
  for (abx in all_abx) {
    ...
    # Check if all genome names are present
    all_genomes_in_folds <- unlist(folds_train_genome_names[[abx]])
    if (!all(all_genomes_in_folds %in% genome_ids)) {
      stop(paste("Some genome names in pre-fold information for", abx, "are not in the current genome_ids"))
    }
    # Convert genome names to indices
    folds_train[[abx]] <- lapply(folds_train_genome_names[[abx]], \(x) {
      match(x, genome_ids)
    })
    # Check number of folds
    if (length(folds_train[[abx]]) != opt$folds) {
      stop(paste("Number of folds in pre-fold information for", abx, "does not match --folds"))
    }
    # Check if folds partition the data correctly
    all_indices <- unlist(folds_train[[abx]])
    if (length(all_indices) != length(genome_ids) || any(duplicated(all_indices))) {
      stop(paste("Pre-fold information for", abx, "does not correctly partition the data"))
    }
  }
} else {
...
```{r load-best-params-kmer, eval=FALSE}
...
```{r train-models-kmers, eval=FALSE}
